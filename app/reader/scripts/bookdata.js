...
var Reader = (function (r) {
	...
	r.Bookmarks = {
		...
		getVisibleBookmarks: function(){
			var bookmarks = [];
			$('[data-bookmark][data-cfi]', r.$body.contents()).each(function(index, el){
		...
		removeBookmark: function(cfi){
			...
			if(chapter !== -1){
				...
				if($.isArray(_bookmarks[chapter]) && index !== -1){
					...
					var $marker = $('[data-bookmark][data-cfi="' + cfi + '"]', r.$body.contents());
					if($marker.length){
		...
		display: function(){
			var isVisible = false;
			$('[data-bookmark]', r.$body.contents()).each(function(index, el){
			...
			if(isVisible){
				$('#cpr-bookmark-ui', r.$body.contents()).show();
				return isVisible;
			} else {
				$('#cpr-bookmark-ui', r.$body.contents()).hide();
			}
			return false;
		}
	};
	...
	r.Highlights = {
		...
		removeHighlight: function(cfi){
			...
			if(chapter !== -1){
				...
				if($.isArray(_highlights[chapter]) && index !== -1){
					...
					$('['+r.Highlights.ATTRIBUTE+'][data-cfi="' + cfi + '"]', r.$body.contents()).remove();
					r.Highlights.display();
					return true;
				}
			}
		...
		display: function(){
			...
			// resize highlights
			$('['+r.Highlights.ATTRIBUTE+']', r.$body.contents()).remove();
			...
			$('['+r.Highlights.ATTRIBUTE+']', r.$body.contents()).each(function(index, el){
				isVisible = r.returnPageElement(el) === r.Navigation.getPage();
				if (isVisible) {
					return false;
				}
